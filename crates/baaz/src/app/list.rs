...
//!
//! Every one of these is a local override written to Baaz's own store
...
impl Harness {
    ...
    /// the minute turns. Under `BAAZ_DETERMINISTIC=1` "now" comes from the
            ...
            wanted
                ...
                .map(|session_id| {
                    ...
                    match read {
                        ...
                        Err(error) => {
                            crate::baaz_log!("session/read for a title failed: {error}");
