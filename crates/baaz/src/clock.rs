...
//! `BAAZ_DETERMINISTIC=1` the clocks freeze so a capture is byte-identical
...
/// Whether `BAAZ_DETERMINISTIC=1` was in the environment, read once.
fn enabled() -> bool {
    static FLAG: OnceLock<bool> = OnceLock::new();
    *FLAG.get_or_init(|| std::env::var("BAAZ_DETERMINISTIC").as_deref() == Ok("1"))
}
