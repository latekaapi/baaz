...
//!
//! Generated by hand from `fixtures/msp/msp-ts/msp.d.ts` (muse 1.0.3) and validated against the
...
//! * **Closed** unions ([`ApprovalMode`], [`HistoryPreference`], [`IfBusy`], [`ReasoningEffort`],
//!   [`TurnInputPartType`], [`UserInputSelectionMode`], [`ViewPageDirection`], [`PlatformFamily`],
//!   [`PlatformOs`], [`CommandAckStatus`], [`JsonRpcVersion`]) have no `Unknown`: an unrecognized
...
//!
//! [`MSP_METHODS`] and [`MSP_NOTIFICATIONS`] are the SS1.9 index verbatim. The muse 1.0.3 binary
...
pub struct Item {
    ...
    ///
    /// **Required-nullable**, against msp.d.ts's `turnId?: string`: every item in every capture
    /// carries the key, explicitly `null` on `userShell`.
    #[serde(default)]
...
/// `session/started` params — **not in the SS1.9 published index**, but emitted by the muse 1.0.3
...
/// The `sha256:` stable-surface fingerprint of the muse 1.0.3 schema bundle these types were
...
///
/// The muse 1.0.3 binary additionally emits `session/started` ([`SessionStartedParams`]), which this
