...
6. **`ReasoningEffort` has `ultra` and no `max`**, while the CLI and the on-disk
   catalog both advertise `max`. Sending `max` is `invalidParams`. The effort
   picker is driven by the MSP enum.
