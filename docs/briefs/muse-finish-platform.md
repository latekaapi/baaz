# Brief — finish Task G — platform of the Harness improvements workflow in its existing worktree
...
existing worktree `/Users/latekaapi/Projects/harness-wt-platform` (branch `wf/platform`, uncommitted
changes present — **inventory them first** with `git status` and `git diff --stat`, read
the diff, and keep what is good). The task specification is section "Task G — platform: menu bar, ⌘W/⌘Q, app bundle and icon" of
...
5. **Commit on `wf/platform`** in the worktree (message ending with
