...
// EXPERIMENTAL surface — includes experimentalApi-gated types
// (calls fail `experimentalRequired` without the opt-in);
// stable twin: ../msp.d.ts.
// Generated by tbh-protocol (spec 206) from the same schema model as
// the JSON Schema bundle (INV-017). Do not edit by hand.

/** `account/loginCancel` result: exactly `{cancelled}` — `true` iff a pending flow existed and was cancelled (spec 28091 FR-29535-5). */
export interface AccountLoginCancelResult {
  /** Whether a pending flow was cancelled. */
  cancelled: boolean;
}

/** `account/loginCompleted` params: exactly `{outcome, message?}` (spec 28091 FR-29535-4). `message` is a display string from the typed error vocabulary — never key material, store content, or an echo of the raw params (FM-29535-4). */
export interface AccountLoginCompletedParams {
  /** Optional display message (`denied`/`expired`/`failed` only, FR-29535-4). */
  message?: string;
  /** How the flow ended. */
  outcome: AccountLoginOutcome;
}

/** How a login flow ended (spec 28091 FR-29535-4). Server-produced, so the vocabulary is open per the #22785 E2 rule — a future flow may end in a new way additively. */
export type AccountLoginOutcome = "granted" | "denied" | "expired" | "cancelled" | "failed" | (string & {});

/** `account/loginStart` params: exactly `{type, apiKey?}` (spec 28091 FR-29535-1). `apiKey` is required non-empty for [`AccountLoginType::ApiKey`] and forbidden for [`AccountLoginType::DeviceCode`]; both violations are invalid-params errors (FM-29535-1), enforced by the handler so the error never echoes the raw params (FM-29535-4). `Debug` is hand-written: the derive would print the raw key bytes into any `{:?}` context (a tracing line, an error, a test assert), the exact leak INV-29535-1 forbids — the `McpOAuthRecordV1` posture. */
export interface AccountLoginStartParams {
  /** The API key to store (the `apiKey` type only). The ONLY secret-bearing member of the account surface: every log sink carries [`ACCOUNT_LOGIN_API_KEY_REDACTED`] in its place (INV-29535-1). */
  apiKey?: string;
  /** Which flow to run. */
  type: AccountLoginType;
}

/** `account/loginStart` result: exactly `{verificationUrl?, userCode?}` — both present for a started device-code flow, both absent for the synchronous `apiKey` type (spec 28091 FR-29535-1). The device-code artifacts travel ONLY here, to the requesting connection; completion notifications never carry them (INV-29535-2). */
export interface AccountLoginStartResult {
  /** The code the user confirms or enters there (device-code flow only). */
  userCode?: string;
  /** Where the user signs in (device-code flow only). */
  verificationUrl?: string;
}

/** Which login flow `account/loginStart` runs (spec 28091 FR-29535-1; ADR D3). Client-produced, so the vocabulary is CLOSED: an unknown `type` fails deserialization into the standard invalid-params error (FM-29535-1). */
export type AccountLoginType = "deviceCode" | "apiKey";

/** The one account payload shape: the `account/read` result and the `account/changed` params (spec 28091 FR-001/FR-002). Closed vocabulary (INV-002); `label` is a display string that never carries key material (FR-004) and is omitted, never `null`, when absent. */
export interface AccountState {
  /** `false` exactly when the effective provider endpoint needs no caller credential (keyless/gateway deployments, FR-005), so those clients never nag the user to sign in. */
  credentialRequired: boolean;
  /** Optional display label for the credential in effect (for example the account identity for `accountLogin`); never key material (FR-004). */
  label?: string;
  /** Which credential lane is in effect. */
  state: AccountStateKind;
}

/** The projected credential state (spec 28091 FR-001; ADR 28091 D2): `EnvKey → envKey`, `StoredKey → apiKey`, `LoginMint → accountLogin`, and the absent lane as `loggedOut`. Open per the #22785 E2 rule — server-produced result vocabularies are open — so a future credential lane can project additively. */
export type AccountStateKind = "loggedOut" | "envKey" | "apiKey" | "accountLogin" | (string & {});
...
export interface ClientCapabilities {
  ...
  /** Whether this client can render a `userInput/request` dialog and answer it (SS5.10). **Absent means capable** (D-047): a client that predates the member keeps today's contract, so no existing client changes its handshake. `false` makes the host withhold `userInput/request` toward this client — the D-027 additive-optional-field + withheld-method convention.  Deliberately NOT a `requestedCapabilities` name: that registry records operations a host GRANTS (SS1.4.4), and whether a client can draw a dialog is a fact about the client, never a privilege the host hands out. */
  userInputDialogs?: boolean;
}
...
export interface ErrorData {
  ...
  approvalId?: string;
  /** The stored output's availability on `outputUnavailable` — the durable `ToolOutputAvailability` value, read-time-derived when the durable record still says `available` (SS4.5.5; spec 208 FM-008). */
  availability?: string;
  ...
  /** The item whose stored output was requested, on the `item/readOutput` arms (`notFound`, `outputUnavailable`, and the misaligned-offset `invalidParams`). */
  itemId?: string;
...
export interface ItemReadOutputParams {
  /** The item whose stored output is fetched. */
  itemId: string;
  /** STORED bytes requested; default and max 6 MiB (tdd SS4.7.4). A value above the max is served at the max. */
  lengthBytes?: number;
  /** STORED-byte offset to start from; 0 when omitted. STORED bytes are a property of the output, not of a transport (tdd SS4.7.4). */
  offsetBytes?: number;
...
export interface ItemReadOutputResult {
  /** STORED bytes served in THIS response (not the output's total; that is `outputRef.byteLen` on the item). The next page's `offsetBytes` is `offsetBytes + byteLen` — short-serving and advancing are separate, so paging never skips or double-reads a byte (tdd SS4.7.4). */
  byteLen: number;
...
export interface SessionModelRouteUnservedParams {
...
export interface ViewSubscribeParams {
  /** Resume cursor. When present, the server replays `(after, head]` as ordinary view notifications before any live event — one gapless cursor sequence. Omitted means "from now": no historical replay (tdd SS4.7.1). */
  after?: string;
...
export interface ViewSubscribeResult {
