...
# login screens. Byte-identical run to run under BAAZ_DETERMINISTIC=1
...
#   scripts/captures.sh <out-dir> [baaz-binary]
set -e
...
for f in fixtures/msp/transcript-*.jsonl fixtures/msp/synthetic-*.jsonl; do
  ...
  for t in dark light; do
    BAAZ_STATE_DIR="$(mktemp -d)" "$bin" --replay "$f" --theme "$t" --screenshot "$out/$name-$t.png" >/dev/null 2>&1 || echo "FAILED $name $t"
  done
done
...
proj_shot() { # <name> <theme> <steps>
  ...
  BAAZ_STATE_DIR="$(mktemp -d)" "$bin" $proj_base --theme "$theme" --steps "$steps" --screenshot "$out/$name.run1.png" >/dev/null 2>&1 || echo "FAILED $name run1"
  BAAZ_STATE_DIR="$(mktemp -d)" "$bin" $proj_base --theme "$theme" --steps "$steps" --screenshot "$out/$name.run2.png" >/dev/null 2>&1 || echo "FAILED $name run2"
...
BAAZ_STATE_DIR="$(mktemp -d)" "$bin" --no-connect --login signed-in --no-project --theme dark --screenshot-delay 15000 --screenshot "$out/projects-hero-dark.run1.png" >/dev/null 2>&1 || echo "FAILED hero run1"
BAAZ_STATE_DIR="$(mktemp -d)" "$bin" --no-connect --login signed-in --no-project --theme dark --screenshot-delay 15000 --screenshot "$out/projects-hero-dark.run2.png" >/dev/null 2>&1 || echo "FAILED hero run2"
